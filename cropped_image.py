...
def crop_plates(image, boxes):
    """Tespit edilen plakaları kırpar ve kaydeder."""
    cropped_images = []
    for i, box in enumerate(boxes):
        x1, y1, x2, y2 = box
        cropped_img = image[y1:y2, x1:x2]
        cropped_images.append(cropped_img)
        cv2.imwrite(f"cropped_plate_{i}.jpg", cropped_img)
        print(f"Plaka {i} kırpıldı ve kaydedildi: cropped_plate_{i}.jpg")
...
def detect_plates(model_path, image_path):
    ...
    if len(boxes) > 0:
        ...
        # Plakaları kırp ve kaydet
        crop_plates(original_img, boxes)
